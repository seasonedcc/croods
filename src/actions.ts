...
const getRequest =
  (store: Store) =>
  ({ operation = 'info', ...options }: ActionOptions): boolean => {
...
const saveRequest =
  (store: Store) =>
  (options: ActionOptions, id?: ID): boolean => {
    const [piece, setState, log] = stateMiddleware(store, options)
  ...
  (
    ...
    if (hasData) {
      ...
      const newState = {
        ...
        ...status,
        list: id
          ? piece.list.map((item: ObjWithId) =>
              sameId(id)(item) ? state : item,
            )
...
const saveFail =
  (store: Store) =>
  (options: ActionOptions, { error, id }: ActionError): boolean => {
    const [piece, setState, log] = stateMiddleware(store, options)
...
const destroySuccess =
  (store: Store) =>
  (options: ActionOptions, id?: ID): Info => {
    const [piece, setState, log] = stateMiddleware(store, options)
...
const setInfo =
  (store: Store) =>
  (options: ActionOptions, info: Info, merge?: boolean): Info => {
    const [piece, setState, log] = stateMiddleware(store, options)
    ...
    const newState = {
      ...
      info: newInfo,
      list: info.id
        ? piece.list.map(replaceItem(info.id, newInfo))
        : piece.list,
    }
...
const setList =
  (store: Store) =>
  (options: ActionOptions, list: Info[], merge?: boolean): Info[] => {
    const [piece, setState, log] = stateMiddleware(store, options)
    const newState = {
      ...piece,
...
const setInfoFromList =
  (store: Store) =>
  (options: ActionOptions): Info | boolean => {
